 ...
 * garbage collected.
 */
static void Instrumenter_dealloc(dw_instrumenter_t inst) {
  dw_instrumenter_destroy(inst);
}

static VALUE Instrumenter_allocate(VALUE self) {
  dw_instrumenter_t inst = dw_instrumenter_init();
  return Data_Wrap_Struct(rb_cInstrumenter, 0, Instrumenter_dealloc, inst);
}

static void Tracer_dealloc(dw_tracer_t tracer) {
  dw_tracer_destroy(tracer);
}

static VALUE Tracer_allocate(VALUE self) {
  dw_tracer_t tracer = dw_tracer_init();
  return Data_Wrap_Struct(rb_cTracer, 0, Tracer_dealloc, tracer);
}

static VALUE Tracer_record(VALUE self, VALUE c, VALUE desc, VALUE annot) {
  return Qnil;
}

static VALUE Tracer_start(VALUE self, VALUE c, VALUE desc, VALUE annot) {
  return Qnil;
}
...
void Init_direwolf_native() {
  ...
  rb_define_public_method(rb_cTracer, "__record", Tracer_record, 3);
  rb_define_public_method(rb_cTracer, "__start",  Tracer_start,  3);
  rb_define_public_method(rb_cTracer, "__stop",   Tracer_stop,   0);
