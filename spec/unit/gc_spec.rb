  ...
  describe GC, :http, :agent do
    ...
    context "when there is no GC and no spans" do
      it "leaves the GC node out" do
        ...
        expect(gc).to receive(:total_time).exactly(2).times.and_return(0.0, 0.0)
    ...
    context "when there is GC and no spans" do
      it "adds a GC node" do
        ...
        expect(gc).to be_enabled
        expect(gc).to receive(:total_time).exactly(2).times.and_return(0.0, 100_000_000)
    ...
    context "when there is GC and a span" do
      it "subtracts GC from the span and adds it at the end" do
        ...
        expect(gc).to be_enabled
        expect(gc).to receive(:total_time).exactly(4).times.and_return(0, 0, 100_000_000, 0)

        Skylight.trace "Rack", "app.rack" do
          Skylight.instrument { clock.skip 1 }
        end
